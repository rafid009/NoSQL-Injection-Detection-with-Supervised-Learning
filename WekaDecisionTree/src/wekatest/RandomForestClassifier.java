...
public class RandomForestClassifier {
    public static void kFoldValidationRF(Instances newData) throws IOException, Exception
    {
        BufferedWriter output = null;
        File file = new File("rf_kFold_stat.txt");
        ...
        for(int l=0;l<100;l++){
            ...
            for(int i=0;i<k;i++)
            {
                ...
                String []options = {"-I", "200"};//{"-P", "-O", "-I", "500", "-num-slots", "0", "-M", "3", "-N", "5", "-B"};
        ...
        System.out.println("TP: "+tPos/100);
        System.out.println("TN: "+tNeg/100);
        System.out.println("FP: "+fPos/100);
        System.out.println("FN: "+fNeg/100);
        ...
        System.out.println("DBG_RF_END\n\n");

        output.flush();
        output.close();
    }

    public static void testingRF(Instances trainData, Instances testData) throws IOException, Exception
    {
        BufferedWriter output = null;
        File file = new File("rf_kFold_stat.txt");
        output = new BufferedWriter(new FileWriter(file));
        ...
        for(int l=0;l<1;l++){
            double TPR=0, TNR=0, FPR=0, FNR=0;
            int TP=0,TN=0,FP=0,FN=0;
            Instances train = new Instances(trainData, 0, trainData.numInstances());
            Instances test = new Instances(testData, 0, testData.numInstances());
